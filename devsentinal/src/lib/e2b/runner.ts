...
/**
 * Run code formatter on changed files inside the sandbox.
 * Detects prettier (JS/TS) or ruff format (Python).
 */
export async function runFormat(
  ...
  if (hasPackageJson) {
    await sandbox.commands.run(
      `cd ${REPO_DIR} && npx prettier --write ${fileList} 2>/dev/null || true`,
      { timeoutMs: 60_000 }
    );
  } else {
    await sandbox.commands.run(
      `cd ${REPO_DIR} && ruff format ${fileList} 2>/dev/null || true`,
      { timeoutMs: 60_000 }
    );
  }
}
